...
export const Cart = observer(({ onAuth }) => {
    ...
    const toWish = async (item) => {
        if (userId) {
            if (!wishList.includes(item.id)) {
                await addToFav(item.id, userId, item.id).then(async () => {
                    ...
                    <div className="CartRow">
                                            ...
                                            <div className="CartItemInfo">
                                                <div className="CartItemIntoTop">
                                                    ...
                                                    {item.category === 'clothes' &&
                                                        <div className="CartItemModel">{item.declension}</div>
                                                    }
                                                    <div className="CartItemSize">Размер: {replaceSize(item.size)} {checkType(item)}</div>
                                                ...
                                                <div className="CartItemInfoBottom">
                                                    {item.category === 'shoes' &&
                                                        <div className="CartItemPrice">{FormatPrice.formatPrice2(FormatPrice.shipPrice(item.price, constants.course, item.ship, constants.standartShip, constants.expressShip, constants.fee))} ₽</div>
                                                    }
                                                    {item.category === 'clothes' &&
                                                        <div className="CartItemPrice">{FormatPrice.formatPrice2(FormatPrice.shipPrice(item.price, constants.course, item.ship, constants.categoriesShips.find(i => i.name === item.brand).standart, constants.categoriesShips.find(i => i.name === item.brand).express, constants.fee))} ₽</div>
                                                    }
                                                </div>
                                            </div>
                        ...
                        <div className="CartOrder">
                            ...
                            <div className="CartOrderCount">
                                ...
                                <div className="CartCountSum">{FormatPrice.formatPrice2(FormatPrice.formatFullArray(cart, constants.course, constants.standartShip, constants.expressShip, constants.fee, constants.categoriesShips).shipSum)} ₽</div>
                            </div>
                            <div className="CartTotal">
                                <div className="CartTotalText">Итого</div>
                                <div className="CartTotalSum">{FormatPrice.formatPrice2(FormatPrice.formatFullArray(cart, constants.course, constants.standartShip, constants.expressShip, constants.fee, constants.categoriesShips).shipSum)} ₽</div>
                            </div>
