...
import "./Order.scss";
import { checkOrderCost, checkOrderSize, fetchCartItems } from "../../http/itemAPI";
...
import { sendCommonOrder } from "../../http/orderAPI";
...
export const Order = observer(() => {
    ...
    const fastShoes = () => {
        return cart.filter((item) => item.ship === 'fast' && item.category === 'shoes')
    }

    const fastClothes = () => {
        return cart.filter((item) => item.ship === 'fast' && item.category === 'clothes')
    }
    ...
    const slowShoes = () => {
        return cart.filter((item) => item.ship === 'slow' && item.category === 'shoes')
    }

    const slowClothes = () => {
        return cart.filter((item) => item.ship === 'slow' && item.category === 'clothes')
    }
    ...
    const checkItems = async () => {
        try {
            const results = await Promise.allSettled(cart.map(cartItem => checkOrderSize(cartItem.item_uid, cartItem.size)))
            const allFailed = results.every(result => result.status === 'rejected')
            if (allFailed) {
                console.error('Все товары закончились:', results)
                setNoSizes(true)
                setCostCheck(true)
            } else {
                console.log('Есть некоторые товары:', results)
                setCostCheck(true)
            }
        } catch (e) {
            setCostCheck(true)
    ...
    const createOrder = async () => {
        ...
        let newFastClothes = []
        for (let i of cart) {
            if (i.ship === 'slow' && i.category === 'shoes') {
                newSlowShoes.push({
                    item_uid: i.item_uid,
                    name: i.name,
                    size: i.size,
                    ...
                    rub_cost: FormatPrice.roundPrice(i.price / 100 * constants.course),
                    fee: constants.fee,
                    delivery_cost: constants.standartShip
                })
            }
            if (i.ship === 'slow' && i.category === 'clothes') {
                newSlowClothes.push({
                    item_uid: i.item_uid,
                    name: i.name,
                    size: i.size,
                    ...
                    rub_cost: FormatPrice.roundPrice(i.price / 100 * constants.course),
                    fee: constants.fee,
                    delivery_cost: constants.categoriesShips.find(j => j.name === i.brand).standart
                })
            }
            if (i.ship === 'fast' && i.category === 'shoes') {
                newFastShoes.push({
                    item_uid: i.item_uid,
                    name: i.name,
                    size: i.size,
            ...
            if (i.ship === 'fast' && i.category === 'clothes') {
                newFastClothes.push({
                    item_uid: i.item_uid,
                    name: i.name,
                    size: i.size,
                    ...
                    rub_cost: FormatPrice.roundPrice(i.price / 100 * constants.course),
                    fee: constants.fee,
                    delivery_cost: constants.categoriesShips.find(j => j.name === i.brand).express
                })
            }
        }
        try {
            const slowOrderShoes = {
                delivery_cost: constants.standartShip,
                cost: FormatPrice.roundPrice(FormatPrice.formatSlowArray(cart.filter(i => i.category === 'shoes'), constants.course, constants.standartShip, constants.fee, constants.categoriesShips).shipSum),
                items: newSlowShoes
            }
            const slowOrderClothes = {
                delivery_cost: constants.standartShip,
                cost: FormatPrice.roundPrice(FormatPrice.formatSlowArray(cart.filter(i => i.category === 'clothes'), constants.course, constants.standartShip, constants.fee, constants.categoriesShips).shipSum),
                items: newSlowClothes
            }
            const fastOrderShoes = {
                delivery_cost: constants.expressShip,
                cost: FormatPrice.roundPrice(FormatPrice.formatFastArray(cart.filter(i => i.category === 'shoes'), constants.course, constants.expressShip, constants.fee, constants.categoriesShips).shipSum),
                items: newFastShoes
            }
            const fastOrderClothes = {
                delivery_cost: constants.expressShip,
                cost: FormatPrice.roundPrice(FormatPrice.formatFastArray(cart.filter(i => i.category === 'clothes'), constants.course, constants.expressShip, constants.fee, constants.categoriesShips).shipSum),
    ...
    return (
        <div className="Order MBInfo">
            ...
            {cart && cart.length === 0 && noSizes &&
                <div className="EmptyCart">
                    <h2>Товара нет в наличии</h2>
                    <p>Мы сожалеем, но данный товар закончился в нужном вам размере.</p>
                ...
                <div className="OrderRow">
                    <div className="OrderInputs">
                        ...
                        <div className="OrderInputsGroup SpaceBetween">
                            ...
                            <div className={`OrderCheckBox FitBox2 ${payment === 'split' ? 'ChosenOrderCheck' : ''} ${FormatPrice.formatFullArray(cart, constants.course, constants.standartShip, constants.expressShip, constants.fee, constants.categoriesShips).shipSum > 20000 ? 'Unavailable' : ''}`} onClick={() => choosePayment('split')}>
                                <div className="CheckBoxRow">
                                    <div className="CheckBoxPoint"></div>
                                    <div className="CheckBoxText">
                        ...
                        <div className="MobileTotal">
                            ...
                            <div className="OrderCount">
                                ...
                                <div className="OrderCountSum">{FormatPrice.formatPrice2(FormatPrice.formatFullArray(cart, constants.course, constants.standartShip, constants.expressShip, constants.fee, constants.categoriesShips).shipSum)} ₽</div>
                            </div>
                            <div className="OrderTotal">
                                <div className="OrderTotalText">Итого</div>
                                <div className="OrderTotalSum">
                                    <span className={`MainSum ${discount > 0 ? 'SumSmall' : ''}`}>{FormatPrice.formatPrice2(FormatPrice.formatFullArray(cart, constants.course, constants.standartShip, constants.expressShip, constants.fee, constants.categoriesShips).shipSum)} ₽</span>
                                    {discount > 0 &&
                                        <span className="DiscountSum">{FormatPrice.formatPrice2(FormatPrice.discountPrice(FormatPrice.formatFullArray(cart, constants.course, constants.standartShip, constants.expressShip, constants.fee, constants.categoriesShips).shipSum, discount))} ₽</span>
                                    }
                                </div>
                            </div>
                            {payment === 'split' &&
                                <>
                                    <div className="OrderSplitInfo">
                                        ...
                                        <div className="OrderSplitSum">{FormatPrice.formatPrice2(FormatPrice.discountSplitPrice(FormatPrice.formatFullArray(cart, constants.course, constants.standartShip, constants.expressShip, constants.fee, constants.categoriesShips).splitShipSum, discount))} ₽</div>
                                    </div>
                                    <div className="OrderSplitInfo">
                    ...
                    <div className="OrderInfo">
                        ...
                        {fastShoes().length > 0 &&
                            <>
                                        ...
                                        return (
                                            <div key={i} className="OrderItem">
                                                ...
                                                <div className="OrderItemInfo">
                                                    <div className="OrderItemInfoTopRow">
                                                        <div className="OrderItemInfoTop">
                                                            ...
                                                            {item.category === 'clothes' &&
                                                                <div className="OrderItemModel">{item.declension}</div>
                                                            }
                                                        </div>
                                                        {(item_uid && size && ship) ?
                                                            <></>
                                                    ...
                                                    <div className="OrderItemInfoBottomRow">
                                                        <div className="OrderItemSize">Размер: {item.size} {item.category === 'shoes' ? 'EU' : ''}</div>
                                                        {item.category === 'shoes' &&
                                                            <div className="OrderItemPrice">{FormatPrice.formatPrice2(FormatPrice.shipPrice(item.price, constants.course, item.ship, constants.standartShip, constants.expressShip, constants.fee))} ₽</div>
                                                        }
                                                        {item.category === 'clothes' &&
                                                            <div className="OrderItemPrice">{FormatPrice.formatPrice2(FormatPrice.shipPrice(item.price, constants.course, item.ship, constants.categoriesShips.find(i => i.name === item.brand).standart, constants.categoriesShips.find(i => i.name === item.brand).express, constants.fee))} ₽</div>
                                                        }
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div className="OrderCount">
                                    <div className="OrderCountText">{cartCount(fastShoes().length)}</div>
                                    <div className="OrderCountSum">{FormatPrice.formatPrice2(FormatPrice.formatFastArray(cart.filter(i => i.category === 'shoes'), constants.course, constants.expressShip, constants.fee, constants.categoriesShips).shipSum)} ₽</div>
                                </div>
                            </>
                        }
                        {fastClothes().length > 0 &&
                            <>
                                <div className="OrderItems">
                                    {fastClothes().map((item, i) => {
                                        return (
                                            <div key={i} className="OrderItem">
                                                <div className="OrderItemImgBox">
                                                    <img className="OrderItemImg" src={item.img} alt="Фото товара" />
                                ...
                                <div className="OrderCount">
                                    <div className="OrderCountText">{cartCount(fastClothes().length)}</div>
                                    <div className="OrderCountSum">{FormatPrice.formatPrice2(FormatPrice.formatFastArray(cart.filter(i => i.category === 'clothes'), constants.course, constants.expressShip, constants.fee, constants.categoriesShips).shipSum)} ₽</div>
                                </div>
                            </>
                        }
                        {slowShoes().length > 0 &&
                            <>
                                ...
                                <div className="OrderCount">
                                    <div className="OrderCountText">{cartCount(slowShoes().length)}</div>
                                    <div className="OrderCountSum">{FormatPrice.formatPrice2(FormatPrice.formatSlowArray(cart.filter(i => i.category === 'shoes'), constants.course, constants.standartShip, constants.fee, constants.categoriesShips).shipSum)} ₽</div>
                                </div>
                            </>
                        }
                        {slowClothes().length > 0 &&
                            <>
                                <div className="OrderItems">
                                    {slowClothes().map((item, i) => {
                                        return (
                                            <div key={i} className="OrderItem">
                                                <div className="OrderItemImgBox">
                                                    <img className="OrderItemImg" src={item.img} alt="Фото товара" />
                                ...
                                <div className="OrderCount">
                                    <div className="OrderCountText">{cartCount(slowClothes().length)}</div>
                                    <div className="OrderCountSum">{FormatPrice.formatPrice2(FormatPrice.formatSlowArray(cart.filter(i => i.category === 'clothes'), constants.course, constants.standartShip, constants.fee, constants.categoriesShips).shipSum)} ₽</div>
                                </div>
                            </>
                        }
