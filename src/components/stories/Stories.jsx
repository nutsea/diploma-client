...
export const Stories = observer(() => {
    ...
    const storyTimer = () => {
        timer = setTimeout(() => {
            if (storyNum === stories.length - 1 || stories.length === 0) {
                setStoryNum(-1)
                    ...
                    <>
                        ...
                        <div className="StoryItems">
                            {stories && stories.map((story, i) => {
                                return (
                                    <div className={`Story ${i === 0 ? 'FirstStory' : ''} ${storyNum === i ? 'ActiveStory' : ''}`} key={i} style={{ backgroundImage: `url(${process.env.REACT_APP_IMG_URL + story.img})` }}>
                                        {storyNum === i &&
