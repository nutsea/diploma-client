                        ...
                        return (
                            <div key={i} className="POrderBox">
                                <div className="POInfo">
                                    <div className="POTop">
                                        <div className="POTitle">№{order.paid > 0 ? 'WP' : 'R'}{order.id} на {FormatPrice.formatPrice(order.discount_cost)} ₽</div>
                                        ...
                                        {order.items.map((item, j) => {
                                            return (
                                                <div key={j} className="POItemCard" onClick={() => handleNavigate(item)}>
                                                    ...
                                                    <div className="POItemPrice">{FormatPrice.formatPrice2(item.rub_cost + item.fee + item.delivery_cost)} ₽</div>
                                                </div>
                                            )
                                        })}
                                    </div>
                                            ...
                                            {!order.is_split ?
                                                ...
                                                <div className="POSplitInfo">
                                                    <div className="POSplitItem">
                                                        ...
                                                        <span>Первая часть сплита - {FormatPrice.formatNoCeil(order.first_pay)} руб.</span>
                                                    </div>
                                                    <div className="POSplitItem">
                                                        {order.second_paid ?
                                                            <img src={split1} alt="Оплачено" />
                                                        ...
                                                        <span>Вторая часть сплита - {FormatPrice.formatNoCeil(order.second_pay)} руб.</span>
                                                    </div>
                                                </div>
                                            }
                                        </span>
                                    </div>
                                    {order.sdek_track && order.status >= 8 && order.status !== 11 &&
                                        <>
                                            ...
                                            <>
                                                {order.buyReport.map((photo, k) => {
                                                    return (
                                                        <div
                                                            key={k}
                                                            className="POReportBuyImgBox"
                                                            onClick={() => {
                                                                setModalReport(order.id)
                                                                setModalReportType('buy')
                                                                setModalReportNum(k)
                                                            }}
                                                        >
                                                            <img src={process.env.REACT_APP_IMG_URL + photo.img} alt="Фото отчет" />
                                                        </div>
                                                    )
                                                })}
                                                {order && order.buyReport && modalReport === order.id && modalReportType === 'buy' &&
                                                    <div
                                            ...
                                            <>
                                                {order.stockReport.map((photo, k) => {
                                                    return (
                                                        <div
                                                            key={k}
                                                            className="POReportStockImgBox"
                                                            onClick={() => {
                                                                setModalReport(order.id)
                                                                setModalReportType('stock')
                                                                setModalReportNum(k)
                                                            }}
                                                        >
                                                            <img src={process.env.REACT_APP_IMG_URL + photo.img} alt="Фото отчет" />
                                                ...
                                                {order && order.stockReport && modalReport === order.id && modalReportType === 'stock' &&
                                                    <div
                                                        className="POSliderModal"
                                                        onClick={(e) => {
                                                            if (!e.target.closest('.POSliderBox')) {
                                                    ...
                                                    >
                                                        <div className="POSliderBox">
                                                            <OrderSlider item={order.stockReport} num={modalReportNum} />
                                                            <img
                                                                className="POSliderClose"
                                                                src={close}
                                                                alt=""
